...
void PlayerController::keyEvent(QKeyEvent event)
{
    ...
    if(event.type() == QEvent::KeyPress) {
        ...
        // player1
            case Qt::Key_W:
                m_players[0].m_keyboardDir |= dir_cast(dir::Up);
                break;
            case Qt::Key_S:
                m_players[0].m_keyboardDir |= dir_cast(dir::Down);
                break;
            case Qt::Key_D:
                m_players[0].m_keyboardDir |= dir_cast(dir::Right);
                break;
            case Qt::Key_A:
                m_players[0].m_keyboardDir |= dir_cast(dir::Left);
                break;
            case Qt::Key_Space:
        ...
        // player2
            case Qt::Key_8:
                if(event.modifiers()){
                    m_players[1].m_keyboardDir |= dir_cast(dir::Up);
                }
                break;
            case Qt::Key_5:
                if(event.modifiers()){
                    m_players[1].m_keyboardDir |= dir_cast(dir::Down);
                }
                break;
            case Qt::Key_6:
                if(event.modifiers()){
                    m_players[1].m_keyboardDir |= dir_cast(dir::Right);
                }
                break;
            case Qt::Key_4:
                if(event.modifiers()){
                    m_players[1].m_keyboardDir |= dir_cast(dir::Left);
                }
                break;
            case Qt::Key_0:
                if(event.modifiers()){
        ...
        // player1
            case Qt::Key_W:
                m_players[0].m_keyboardDir &= ~static_cast<ushort>(dir::Up);
                break;
            case Qt::Key_S:
                m_players[0].m_keyboardDir &= ~static_cast<ushort>(dir::Down);
                break;
            case Qt::Key_D:
                m_players[0].m_keyboardDir &= ~static_cast<ushort>(dir::Right);
                break;
            case Qt::Key_A:
                m_players[0].m_keyboardDir &= ~static_cast<ushort>(dir::Left);
                break;
        // player2
            case Qt::Key_8:
                if(event.modifiers()){
                    m_players[1].m_keyboardDir &= ~static_cast<ushort>(dir::Up);
                }
                break;
            case Qt::Key_5:
                if(event.modifiers()){
                    m_players[1].m_keyboardDir &= ~static_cast<ushort>(dir::Down);
                }
                break;
            case Qt::Key_6:
                if(event.modifiers()){
                    m_players[1].m_keyboardDir &= ~static_cast<ushort>(dir::Right);
                }
                break;
            case Qt::Key_4:
                if(event.modifiers()){
                    m_players[1].m_keyboardDir &= ~static_cast<ushort>(dir::Left);
                }
                break;
            case Qt::Key_Space:
                break;
...
{
    auto time = clock::now();
    qDebug() << "time: " << (time - dt.m_timeLastShoot).count();
...
bool matchesDir(dir target, ushort container) {
    return bool(dir_cast<ushort>(target) & container);
}

void PlayerController::movePlayer(PlayerController::data &dataPl)
{
    // если игрок направлен куда то и нажата туда же кнопка, то перемещать туда
    if(matchesDir(dataPl.m_pl->m_dir, dataPl.m_keyboardDir)) {
        qreal X, Y;
        switch (dataPl.m_pl->m_dir) {
            case dir::Down:
            case dir::Up:
                X = dataPl.m_pl->x();
                Y = dataPl.m_pl->y()
                        -SPEED_H * bool(dataPl.m_keyboardDir & dir_cast(dir::Up))
                        +SPEED_H * bool(dataPl.m_keyboardDir & dir_cast(dir::Down));
                break;
            ...
            case dir::Right:
                Y = dataPl.m_pl->y();
                X = dataPl.m_pl->x()
                        -SPEED_W * bool(dataPl.m_keyboardDir & dir_cast(dir::Left))
                        +SPEED_W * bool(dataPl.m_keyboardDir & dir_cast(dir::Right));
                break;

            default:
                throw std::logic_error("PlayerController::movePlayer - default branch");
                break;
        }

        if(!qFuzzyCompare(Y, dataPl.m_pl->y()) && !qFuzzyCompare(X, dataPl.m_pl->x()) &&
                0 <= Y && Y <= (m_rectScene.height() - dataPl.m_pl->height()) &&
                0 <= X && X <= (m_rectScene.width() - dataPl.m_pl->width())) {

//            m_scene->items()
            auto list = m_scene->items({X, Y});
    ...
    if(!qFuzzyCompare(Y, dataPl.m_pl->y()) &&
        ...
        for(auto item : list) {
            if(item == dataPl.m_pl
                    || item->type() == static_cast<int>(typeItems::ignoreCollize)) {
                continue;
            }
